...
import { ArrowUpDown, Building2, Image as ImageIcon, Plus, Search, Users } from "lucide-react"
...
const columns: ColumnDef<Property>[] = [
  ...
  },
  {
    ...
    header: "เนื้อที่",
    cell: ({ row }) => (
      <span className="text-sm text-muted-foreground">{row.original.data?.area ?? "—"}</span>
    ),
  },
  {
    id: "extras",
    ...
    <div className="flex flex-col gap-6">
      ...
      {error && (
        <div className="rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">
          ดึงข้อมูลไม่สำเร็จ — {error instanceof Error ? error.message : String(error)}
        </div>
      )}
