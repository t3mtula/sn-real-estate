...
export function Landlords() {
  ...
  const rows = useMemo(() => {
    if (!landlords) return []
    if (!contracts)
      return landlords.map((l) => ({ ...l, _contractCount: 0 }))
    return landlords.map((l) => ({
      ...l,
      _contractCount: countContracts(l, contracts),
    }))
  }, [landlords, contracts])

  const columns = useMemo<
    ColumnDef<Landlord & { _contractCount: number }>[]
  >(
    () => [
      {
        id: 'name',
      ...
      {
        id: 'banks',
        accessorFn: (row) => getLandlordBankCount(row.data),
        header: 'บัญชี',
        cell: ({ row }) => {
          const n = getLandlordBankCount(row.original.data)
          return (
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _id, filterValue) => {
      ...
      const haystack = [
        ...
        t?.addrDistrict,
        t?.addrSubdistrict,
        ...((t?.banks ?? []).flatMap((b) => [b.bank, b.acctNo, b.accountName])),
      ]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
