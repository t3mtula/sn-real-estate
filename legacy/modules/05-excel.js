...
function exportExcelAll(){
  ...
  const conRows=DB.contracts.map(c=>{
    ...
    return {
      ...
      'เงินประกัน':c.deposit||'','ปรับค่าเช่า':c.rateAdj||'','ปรับค่าเช่า_ประเภท':c.rateAdjType||'','ปรับค่าเช่า_เปอร์เซ็นต์':c.rateAdjPercent||0,
...
function exportExcelContracts(){
  ...
  const rows=DB.contracts.map(c=>{
    ...
    return {
      ...
      'ปรับค่าเช่า':c.rateAdj||'','ปรับค่าเช่า_ประเภท':c.rateAdjType||'','ปรับค่าเช่า_เปอร์เซ็นต์':c.rateAdjPercent||0,
      'ผู้ให้เช่า':c.landlord||'','ที่อยู่ผู้ให้เช่า':c.landlordAddr||'',
      'ธนาคาร':c.bank||'','เลขบัญชี':c.acctNo||'','ชื่อบัญชี':c.accountName||'',
      'สถานะ':s==='active'?'ใช้งาน':s==='expired'?'หมดอายุ':s==='soon'?'ใกล้หมด':s==='cancelled'?'ยกเลิก':'รอเริ่ม',
    ...
    try{
      ...
      if(conSheet.length){
        DB.contracts=conSheet.map(r=>({
          ...
          deposit:r['เงินประกัน']||'',rateAdj:r['ปรับค่าเช่า']||'',rateAdjType:r['ปรับค่าเช่า_ประเภท']||(r['ปรับค่าเช่า']?'custom':'none'),rateAdjPercent:parseInt(r['ปรับค่าเช่า_เปอร์เซ็นต์'])||0,
