...
//  - allowed=true หมายถึง email อยู่ใน re_staff (หรืออยู่ใน bootstrap mode)
//  - allowed=false หมายถึง re_staff non-empty + email ไม่อยู่ในนั้น → DENY
async function _resolveStaffRole(email, name) {
  try {
    ...
    if(!all || all.length === 0) {
      ...
      if(!ins.ok) {
        // RLS reject — แสดงว่า bootstrap path ไม่ผ่าน, treat as denied
        return { allowed:false, role:null };
      }
      return { allowed:true, role:'admin' };
    }

    // 3. ไม่อยู่ใน re_staff + table มีคนอยู่แล้ว → ปฏิเสธ
    return { allowed:false, role:null };
  } catch(e) {
