  ...
  $('contractForm').addEventListener('submit', e => {
    ...
    confirmOverlapAndSave(thePid,fd.get('start'),fd.get('end'),null,()=>{
      ...
      const newContract = {
        ...
        accountName: fd.get('accountName') || '', rateAdj: fd.get('rateAdj') || '', rateAdjType: fd.get('rateAdjType') || 'none', rateAdjPercent: parseInt(fd.get('rateAdjPercent'))||0,
  ...
  $('contractForm').addEventListener('submit', e => {
    ...
    confirmOverlapAndSave(thePid,fd.get('start'),fd.get('end'),null,()=>{
      ...
      const newContract = {
        ...
        property: DB.properties.find(x => x.pid === thePid)?.name || '',
        area: fd.get('area') || '', spot: fd.get('spot') || '', landlord: shortLandlordName(fd.get('landlord') || ''), landlordAddr: assembleAddrFromPrefix(fd,'la'),
        accountName: fd.get('accountName') || '', rateAdj: fd.get('rateAdj') || '', rateAdjType: fd.get('rateAdjType') || 'none', rateAdjPercent: parseInt(fd.get('rateAdjPercent'))||0, deed: fd.get('deed') || '',
        dueDay: clampInt(fd.get('dueDay'),{min:1,max:31,def:5}),
        copiedFrom: cid,
        invHeaderId: fd.get('invHeaderId') || null,
        tenantLogo: fd.get('tenantLogo') || null,
      ...
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:14px 16px;margin-top:4px">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="col-span-2">
            ...
            ${(()=>{
              // Initial state — รองรับ legacy (มีแต่ c.rateAdj text) + structured ใหม่ (c.rateAdjType + c.rateAdjPercent)
              const curText = v('rateAdj') || '';
              ...
              <div style="display:flex;flex-direction:column;gap:6px;font-size:13px;color:#374151">
                <label style="cursor:pointer;display:flex;align-items:center;gap:6px">
                  <input type="radio" name="rateAdjType" value="none" ${curType==='none'?'checked':''} onchange="rateAdjOnChange()">
                  <span>ไม่ปรับ (ค่าเช่าคงที่ตลอดสัญญา)</span>
                </label>
                <label style="cursor:pointer;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
                  <input type="radio" name="rateAdjType" value="percent" ${curType==='percent'?'checked':''} onchange="rateAdjOnChange()">
                  <span>ปรับเมื่อต่อสัญญา</span>
                  <input type="number" name="rateAdjPercent" value="${curPct}" min="1" max="100" step="1" oninput="rateAdjOnChange()" style="width:60px;padding:3px 6px;border:1px solid #cbd5e1;border-radius:4px;text-align:center">
                  <span>%</span>
                </label>
                <label style="cursor:pointer;display:flex;align-items:center;gap:6px">
                  <input type="radio" name="rateAdjType" value="custom" ${curType==='custom'?'checked':''} onchange="rateAdjOnChange()">
                  <span>พิมพ์เอง</span>
                </label>
              </div>
...
function rateAdjOnChange(){
  const typeEl = document.querySelector('input[name=rateAdjType]:checked');
  if(!typeEl) return;
  const type = typeEl.value;
  const pctEl = document.querySelector('input[name=rateAdjPercent]');
  const textEl = document.querySelector('input[name=rateAdjText]');
  const hiddenEl = document.querySelector('input[name=rateAdj]');
  const preview = document.getElementById('rateAdjPreview');
  const previewText = document.getElementById('rateAdjPreviewText');
  if(!hiddenEl) return;
  // Toggle visibility: custom text vs preview
  if(textEl) textEl.style.display = (type === 'custom') ? '' : 'none';
  if(preview) preview.style.display = (type === 'custom') ? 'none' : '';
  // Generate text based on type
  let text = '';
  if(type === 'none'){
    text = 'ไม่มีการปรับอัตราค่าเช่า ตลอดอายุสัญญา';
...
function resolveFormDropdowns(fd){
  ...
  // RateAdj (new structured): hidden rateAdj already has generated text from rateAdjOnChange();
  // here ensure it's filled in case JS didn't run (e.g. headless / no interaction)
  const _rateAdjType = fd.get('rateAdjType') || 'none';
  if(!fd.get('rateAdj')){
